...
async function runTick(): Promise<void> {
  ...
  for (const stock of allStocks) {
    ...
    const volatility = Number(stock.volatilityFactor) || 0.02;
    const prevPrice = Number(quote.price);
    const newPrice = randomWalkStep(prevPrice, volatility * 0.6, Math.random);
    const prevClose = Number(quote.prevClose);
    const high = round(Math.max(Number(quote.high), newPrice));
    const low = round(Math.min(Number(quote.low), newPrice));
    const changePct = round(((newPrice - prevClose) / prevClose) * 100, 3);
    const volumeLots = round(Number(quote.volumeLots) + Math.random() * 40, 2);
    const amountWan = round((volumeLots * 100 * newPrice) / 10, 2);

    const closes = closesByStock.get(stock.id) ?? [prevPrice];
